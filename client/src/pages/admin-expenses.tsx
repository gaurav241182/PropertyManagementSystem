...
import {
  ...
  CheckCheck, Ban, AlertCircle, Lock, ArrowUpDown, SendHorizonal, SquareCheck, RotateCcw
} from "lucide-react";
...
function validateFile(f: File): string | null {
  if (f.size > MAX_FILE_SIZE) return `"${f.name}" exceeds 5 MB (${(f.size/1024/1024).toFixed(1)} MB).`;
  if (!ALLOWED_TYPES.includes(f.type)) return `"${f.name}" must be PDF, JPEG, PNG, WebP, or GIF.`;
  return null;
}
...
function RowStatusActions({ status, isOwner, onChange }: {
  ...
  if (status === "Submitted") {
    if (isOwner) return (
      <div className="flex items-center gap-0.5">
        <button onClick={() => onChange("Paid")}    title="Approve — Mark Paid" className="h-6 w-6 rounded flex items-center justify-center text-green-600 hover:bg-green-100 transition-colors"><CheckCheck className="h-3.5 w-3.5"/></button>
        <button onClick={() => onChange("Rejected")} title="Reject"             className="h-6 w-6 rounded flex items-center justify-center text-red-600   hover:bg-red-100   transition-colors"><Ban       className="h-3.5 w-3.5"/></button>
        <button onClick={() => onChange("Pending")}  title="Revert to Pending"  className="h-6 w-6 rounded flex items-center justify-center text-amber-600 hover:bg-amber-100 transition-colors"><RotateCcw className="h-3 w-3"/></button>
      </div>
    );
    return <StatusBadge status={status} />;
  }

  // Paid or Rejected — owner can revert
  if (isOwner) return (
    <div className="flex items-center gap-1">
      <StatusBadge status={status} />
      <button onClick={() => onChange("Pending")} title="Revert to Pending"
        className="h-5 w-5 flex items-center justify-center rounded text-muted-foreground hover:text-amber-600 hover:bg-amber-50 transition-colors">
...
function DeleteButton({ onDelete, disabled }: { onDelete: ()=>void; disabled: boolean }) {
  ...
  if (c) return (
    <div className="flex items-center gap-0.5 bg-red-50 border border-red-200 rounded px-1 py-0.5">
      ...
      <button className="text-green-600 p-0.5" onClick={()=>{setC(false);onDelete();}}><CheckCheck className="h-3 w-3"/></button>
      <button className="text-muted-foreground p-0.5" onClick={()=>setC(false)}><X className="h-3 w-3"/></button>
    </div>
  );
  return (
    <button className="h-7 w-7 flex items-center justify-center text-muted-foreground hover:text-red-500 rounded"
      onClick={()=>setC(true)} disabled={disabled} title="Delete row"><Trash2 className="h-3.5 w-3.5"/></button>
  );
}
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-background rounded-t-2xl sm:rounded-xl shadow-2xl w-full sm:max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b">
          <div className="flex items-center gap-2"><SendHorizonal className="h-5 w-5 text-primary"/><h2 className="text-base font-bold">Submit for Approval</h2></div>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          {zeroPriceItems.length>0 && (
            <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5">
              ...
              <div><p className="text-xs font-semibold text-amber-700">Missing prices</p><p className="text-xs text-amber-600">{zeroPriceItems.length} item(s) have zero price. Please fill in the cost before submitting.</p></div>
            </div>
          )}
          <div>
              ...
              : <div className="divide-y rounded-lg border overflow-hidden">
                  {Object.entries(catBreakdown).map(([cat,{count,total}]) => (
                    <div key={cat} className="flex items-center justify-between px-3 py-2 bg-background">
                      <div><span className="text-sm font-medium">{cat}</span><span className="text-xs text-muted-foreground ml-2">{count} item{count!==1?"s":""}</span></div>
          ...
          <p className="text-xs text-muted-foreground">Once submitted, all pending items will be locked for editing. The owner will review and approve or reject them.</p>
        </div>
        <div className="flex gap-3 px-5 pb-5 pt-2">
          <Button variant="outline" className="flex-1" onClick={onClose}>Cancel</Button>
          <Button className="flex-1" disabled={toSubmit.length===0||zeroPriceItems.length>0} onClick={onConfirm}>
            <SendHorizonal className="mr-2 h-4 w-4"/>Submit {toSubmit.length} Item{toSubmit.length!==1?"s":""}
          </Button>
        </div>
      </div>
    </div>
...
/* ─── Desktop table row ─── */
function ExpenseRow({ serialNo, expense, isOwner, hasEditAccess, onUpdate, onDelete, isDeleting,
  categorySubs, categoryItems, taxableTypes, isSelected, onSelect }: {
  serialNo: number; expense: Expense; isOwner: boolean; hasEditAccess: boolean;
  onUpdate: (id:number,d:Record<string,any>)=>void; onDelete: (id:number)=>void; isDeleting: boolean;
  ...
  const locked     = expense.status !== "Pending";          // field editing only for Pending
  const canEdit    = hasEditAccess && !locked;              // can edit fields
  ...
  return (
    <TableRow className={`hover:bg-muted/10 ${rowBg}`}>
      <TableCell className="p-1 text-center"><Checkbox checked={isSelected} onCheckedChange={c=>onSelect(expense.id,!!c)} disabled={locked}/></TableCell>
      ...
      <TableCell className="p-1">
        {expense.subCategory==="Others"||!(categoryItems[expense.category]?.[expense.subCategory]?.length)
          ? <Input value={localItem} onChange={e=>{if(canEdit)setLocalItem(e.target.value);}} onBlur={()=>canEdit&&handleBlur("item",localItem)} className="h-7 w-full text-sm" placeholder="Item" disabled={!canEdit}/>
          : <Select value={expense.item||""} onValueChange={v=>{if(canEdit){setLocalItem(v);onUpdate(expense.id,{item:v});}}} disabled={!canEdit}>
              <SelectTrigger className="h-7 w-full text-sm" disabled={!canEdit}><SelectValue placeholder="Select"/></SelectTrigger>
      ...
      <TableCell className="p-1">
        <RowStatusActions status={expense.status} isOwner={isOwner} onChange={s=>onUpdate(expense.id,{status:s})}/>
      </TableCell>
      <TableCell className="p-1 text-center">
        {expense.status==="Pending" && hasEditAccess && (
...
/* ─── Mobile card ─── */
function MobileExpenseCard({ expense, serialNo, isOwner, hasEditAccess, onUpdate, onDelete, isDeleting,
  categorySubs, categoryItems, taxableTypes, isSelected, onSelect }: {
  serialNo: number; expense: Expense; isOwner: boolean; hasEditAccess: boolean;
  onUpdate: (id:number,d:Record<string,any>)=>void; onDelete: (id:number)=>void; isDeleting: boolean;
  categorySubs: Record<string,string[]>; categoryItems: Record<string,Record<string,string[]>>;
  ...
  const canEdit   = hasEditAccess && !locked;
  const isTax     = (cat:string) => taxableTypes.has(cat);
  const hasFile   = !!(expense as any).receiptFileName;
  const zeroPWarn = isZeroPriceRow(expense);
    ...
    <div className={`border-2 rounded-xl mx-3 mb-3 overflow-hidden ${cardBg}`} data-testid={`card-expense-${expense.id}`}>
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-black/5">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          {!locked && hasEditAccess && <Checkbox checked={isSelected} onCheckedChange={c=>onSelect(expense.id,!!c)}/>}
        ...
        <div className="flex items-center gap-1.5 shrink-0 ml-2">
          <RowStatusActions status={expense.status} isOwner={isOwner} onChange={s=>onUpdate(expense.id,{status:s})}/>
          {expense.status==="Pending" && hasEditAccess && <DeleteButton onDelete={()=>onDelete(expense.id)} disabled={isDeleting}/>}
        </div>
      </div>
      ...
      <div className={`p-3 space-y-2.5 ${!canEdit?"opacity-60 pointer-events-none":""}`}>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            ...
            {expense.subCategory==="Others"||!(categoryItems[expense.category]?.[expense.subCategory]?.length)
              ? <Input value={localItem} onChange={e=>setLocalItem(e.target.value)} onBlur={()=>handleBlur("item",localItem)} className="h-9 w-full text-sm" placeholder="Item" disabled={!canEdit}/>
              : <Select value={expense.item||""} onValueChange={v=>{setLocalItem(v);onUpdate(expense.id,{item:v});}} disabled={!canEdit}>
                  <SelectTrigger className="h-9 w-full text-sm"><SelectValue placeholder="Select"/></SelectTrigger>
                  <SelectContent>{(categoryItems[expense.category]?.[expense.subCategory]||[]).map(it=><SelectItem key={it} value={it}>{it}</SelectItem>)}</SelectContent>
                </Select>
            }
...
export default function AdminExpenses() {
  ...
  /* ── Auth & permissions ── */
  const { user } = useAuth();
  const { canEdit: canEditPerm, canCreate, isOwnerOrAdmin } = usePermissions();
  const isOwner       = isOwnerOrAdmin;                              // owner / super_admin — full control
  const hasEditAccess = isOwner || canEditPerm("expenses");          // can add / edit / submit
  const hasCreateAccess = isOwner || canCreate("expenses");          // can add rows
  ...
  const categorySubs = useMemo(()=>{
    ...
    categories.forEach((c:any)=>{ if(!map[c.type])map[c.type]=[]; if(c.subtype&&!map[c.type].includes(c.subtype))map[c.type].push(c.subtype); });
  ...
  const categoryItems = useMemo(()=>{
    ...
    categories.forEach((c:any)=>{ if(!map[c.type])map[c.type]={}; if(!map[c.type][c.subtype])map[c.type][c.subtype]=[]; if(c.item&&!map[c.type][c.subtype].includes(c.item))map[c.type][c.subtype].push(c.item); });
    return map;
  },[categories]);
  ...
  const selectableMobile  = visibleExpenses.filter(e=>e.status==="Pending"&&hasEditAccess);
  ...
  const selectableDesktop  = visibleExpenses.filter(e=>e.status==="Pending"&&hasEditAccess);
  ...
  const handleBulkFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for(const f of valid){ const b64=await fileToBase64(f); await addFile.mutateAsync({recordDate,fileName:f.name,fileType:f.type||"application/octet-stream",fileData:b64}); }
    if(valid.length) toast({title:"Files uploaded",description:`${valid.length} file(s) saved.`});
    e.target.value="";
  };

  if(isLoading) return <AdminLayout><div className="flex items-center justify-center h-64 text-muted-foreground">Loading…</div></AdminLayout>;

  const someSelected = selectedIds.size>0;
  const sharedProps = { isOwner, hasEditAccess, onUpdate:handleUpdate, onDelete:handleDelete, isDeleting:deleteExp.isPending, categorySubs, categoryItems, taxableTypes };
        ...
        <Card>
          ...
          <CardHeader className="pb-0 border-b bg-muted/20 px-3 sm:px-5 pt-3">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2"><FileSpreadsheet className="h-5 w-5 text-green-600 shrink-0"/><CardTitle className="text-base">Daily Expense Sheet</CardTitle></div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {dailyFiles.map(f=>(
                    <div key={f.id} className="flex items-center gap-1.5 bg-background border rounded-md px-2 py-1">
                      ...
                      {isOwner && <button className="text-red-400 hover:text-red-600" onClick={()=>delFile.mutate(f.id)}><X className="h-3 w-3"/></button>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="flex items-center justify-between px-3 py-2 bg-muted/10 border-b gap-2 flex-wrap">
              {someSelected ? (
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {isOwner && (
                    <>
                      <Button size="sm" variant="outline" className="h-7 text-xs text-green-700 border-green-300 hover:bg-green-50"
                        onClick={()=>{ const ids=visibleExpenses.filter(e=>selectedIds.has(e.id)&&e.status==="Submitted").map(e=>e.id); Promise.all(ids.map(id=>updateExp.mutateAsync({id,status:"Paid"}))).then(()=>{toast({title:`${ids.length} approved`});setSelectedIds(new Set());}); }}>
                      ...
                      <Button size="sm" variant="outline" className="h-7 text-xs text-red-700 border-red-300 hover:bg-red-50"
                        onClick={()=>{ const ids=visibleExpenses.filter(e=>selectedIds.has(e.id)&&e.status==="Submitted").map(e=>e.id); Promise.all(ids.map(id=>updateExp.mutateAsync({id,status:"Rejected"}))).then(()=>{toast({title:`${ids.length} rejected`});setSelectedIds(new Set());}); }}>
                      ...
                      <Button size="sm" variant="outline" className="h-7 text-xs text-amber-700 border-amber-300 hover:bg-amber-50"
                        onClick={()=>{ const ids=visibleExpenses.filter(e=>selectedIds.has(e.id)&&(e.status==="Paid"||e.status==="Rejected"||e.status==="Submitted")).map(e=>e.id); Promise.all(ids.map(id=>updateExp.mutateAsync({id,status:"Pending"}))).then(()=>{toast({title:`${ids.length} reverted`});setSelectedIds(new Set());}); }}>
                  ...
                  {hasEditAccess && (
                    <Button size="sm" variant="outline" className="h-7 text-xs text-red-700 border-red-300 hover:bg-red-50"
                      onClick={async()=>{ const ids=visibleExpenses.filter(e=>selectedIds.has(e.id)&&e.status==="Pending").map(e=>e.id); await Promise.all(ids.map(id=>deleteExp.mutateAsync(id))); toast({title:`${ids.length} deleted`}); setSelectedIds(new Set()); }}>
            ...
            <div className="block md:hidden">
              ...
              <div className="py-2">
                {visibleExpenses.length===0
                  ? <div className="text-center text-muted-foreground py-10 text-sm">{filterStatus!=="All"?`No ${filterStatus} expenses on ${recordDate}.`:`No expenses for ${recordDate}. Tap Add Line Item to start.`}</div>
                ...
                <TableHeader>
                  <TableRow className="bg-muted/50 hover:bg-muted/50">
                    <TableHead className="p-1 text-center">
                      <Checkbox checked={allDesktopSelected} onCheckedChange={c=>{ setSelectedIds(c?new Set(selectableDesktop.map(e=>e.id)):new Set()); }} data-testid="checkbox-select-all"/>
                    </TableHead>
